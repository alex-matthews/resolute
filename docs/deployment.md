...
- **Secret** `resolute-household-secret` (generated by the
  `resolute-household` ExternalSecret): the household preference prose
  mounted at `/config/household.md` (ADR-0003). A Secret, not a git
  ConfigMap, because it names household members. Source: a
  `RESOLUTE_HOUSEHOLD_PROSE` field on the `resolute` 1Password item —
  create it before deploying v2 or the pod fails fast at startup. Editing
  it and letting reloader roll the pod is the whole tuning loop.
...
docker run -p 8080:8080 -p 8081:8081 \
  ...
  -e RESOLUTE_JUDGE__ENABLED=true \
  -e RESOLUTE_JUDGE__BASE_URL=http://litellm.local/v1 \
  -e RESOLUTE_JUDGE__API_KEY=... \
  resolute
# Without the judge env, resolute runs but degraded (ADR-0003): every
# decision is the conservative 1080p + hold fallback.
```
